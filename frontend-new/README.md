...
5. Set up the required environment variables:
   ...
   - The following environment variables can be set in the `env.js` file:
     ...
     - `GLOBAL_ENABLE_CV_UPLOAD`: (**Optional**) A boolean value to enable or disable CV upload functionality. Defaults to `false` if not set.
     ...
     - `GLOBAL_DISABLE_LOGIN_CODE`: (**Optional**) A boolean value to disable the login code prompt. If set to true, we disable the anonymous login code prompt and the user will only be able to log in using an existing account.
     ...
     - `GLOBAL_DISABLE_REGISTRATION_CODE`: (**Optional**) A boolean value to hide the registration code input field on the Register page. When set to true, the registration code input is not displayed, and users can register without entering a code (backend validation must also be disabled via `GLOBAL_DISABLE_REGISTRATION_CODE`). This is distinct from `FRONTEND_DISABLE_REGISTRATION` which disables registration entirely. Use this for testing, demo environments, or deployments with external access control.
