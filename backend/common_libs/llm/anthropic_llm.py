...
def _normalize_schema(node):
    ...
    if isinstance(node, dict):
        ...
        for k, v in node.items():
            if k == "type_":
                out["type"] = v.lower() if isinstance(v, str) else v
            elif k == "format_":
                out["format"] = v
            elif k == "nullable":
...
class AnthropicLLM(LLM):
    ...
    async def _internal_generate_content(self, llm_input: LLMInput | str) -> LLMResponse:
        ...
        if "temperature" in self._generation_config:
            kwargs["extra_body"] = {"temperature": self._generation_config["temperature"]}
        elif "top_p" in self._generation_config:
            kwargs["extra_body"] = {"top_p": self._generation_config["top_p"]}
    ...
    def _build_messages(self, llm_input: LLMInput | str) -> list[dict]:
        ...
        for turn in llm_input.turns:
            # Anthropic rejects turns with empty content — skip them.
            if not turn.content:
                continue
